                        ...
                        <li className="py-3 sm:py-4">
                            <div className="flex items-center space-x-4">
                                <div className="shrink-0">
                                    {account.type == 'card' ? <FaCreditCard size={20} /> : <PiMoneyWavy size={20} /> }
                                </div>
                                <div className="min-w-0 flex-1">
                                    <p className="truncate text-sm font-medium text-gray-900 dark:text-white">{account.name}</p>

                                    {account.type == 'card' && <p className="truncate text-sm text-gray-500 dark:text-gray-400">{formatCardNumber(account.card_number)}</p>}
                                </div>
